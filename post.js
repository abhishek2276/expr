...
const createPost=(request,response)=>{
   ...
   connection.query('insert into post (truckNumber, date, time, `from`, `to`, loadingSublocations,unloadingSublocations, crn ) values(?,?,?,?,?,?,?,?)',[ truckNumber,  date, time,from, to,loadingSublocations,unloadingSublocations,crn],(error,results)=>{
    if(error){
        throw error
    }
...
const createPost1=(request,response)=>{
    ...
    connection.query('insert into post1 (truckNumber, date, time,`from`, `to`, loadingSublocations,unloadingSublocations, crn ) values(?,?,?,?,?,?,?,?)',[ truckNumber,  date, time,from, to,loadingSublocations,unloadingSublocations,crn],(error,results)=>{
     if(error){
         throw error
     }
     response.status(200).send(`post truck add with id:${results.insertId}`)
