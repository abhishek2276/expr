...
const getTrucks = (request, response) => {
  ...
  connection .query(
    'SELECT t1.truckNumber, t1.truckMaxWeight, t1.truckWheels, t1.rightside, t2.time, t2.from, t2.to, t2.date, t2.loadingSublocations, t2.unloadingSublocations FROM truck AS t1 JOIN post1 AS t2 ON t1.truckNumber = t2.truckNumber WHERE t2.crn = ?',
    [crn],
    (error, results) => {
      if (error) {
        throw error;
